...
for node in $node_list; do
    $SRUN_HEADER --container-name=$CONTAINER_NAME --nodes=1 --ntasks-per-node=1 -w $node --output=slurm-$SLURM_JOB_ID-$node-tritonserver-log.out --mpi=pmix /work/gb200_repro/run_server_nvl4.sh &
done
